@echo off
REM Simple Complete Backup Script for iska-RMS (Fixed Version)
...
REM Backup documentation files using PowerShell for better path handling
echo 📄 Backing up documentation files...
powershell -Command "Get-ChildItem -Path '.' -Recurse -Filter '*.md' | Where-Object { $_.FullName -notlike '*node_modules*' -and $_.FullName -notlike '*.git*' -and $_.FullName -notlike '*backups*' -and $_.FullName -notlike '*%CODEBASE_BACKUP_FOLDER%*' } | ForEach-Object { $relativePath = $_.FullName.Substring((Get-Location).Path.Length + 1); $targetDir = Split-Path -Parent (Join-Path '%CODEBASE_BACKUP_FOLDER%' $relativePath); if (!(Test-Path $targetDir)) { New-Item -ItemType Directory -Path $targetDir -Force | Out-Null }; Copy-Item $_.FullName (Join-Path '%CODEBASE_BACKUP_FOLDER%' $relativePath) -Force }"
echo ✅ Backed up documentation files

REM Backup utility scripts using PowerShell for better path handling
echo 🔧 Backing up utility scripts...
powershell -Command "Get-ChildItem -Path '.' -Recurse -Filter '*.js' | Where-Object { $_.FullName -notlike '*node_modules*' -and $_.FullName -notlike '*.git*' -and $_.FullName -notlike '*backups*' -and $_.FullName -notlike '*%CODEBASE_BACKUP_FOLDER%*' } | ForEach-Object { $relativePath = $_.FullName.Substring((Get-Location).Path.Length + 1); $targetDir = Split-Path -Parent (Join-Path '%CODEBASE_BACKUP_FOLDER%' $relativePath); if (!(Test-Path $targetDir)) { New-Item -ItemType Directory -Path $targetDir -Force | Out-Null }; Copy-Item $_.FullName (Join-Path '%CODEBASE_BACKUP_FOLDER%' $relativePath) -Force }"
...
echo   "systemInfo": {
echo     "os": "Windows",
echo     "backupScript": "simple-backup-fixed.bat"
echo   }
echo }
